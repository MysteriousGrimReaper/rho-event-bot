...
module.exports = {
	...
	async execute(interaction) {
		...
		const death_messages = ["was struck by lightning!"];
		const death_ids = [
			"772156111465349121",
			"241371232983973888",
			"357983991296557056",
		...
		const interval = setInterval(async function () {
			console.log(`hi`);
			/*participants = interaction.guild.roles.cache.find(
			...
			await db.all().then(async (a) => {
				...
				await elim_channel
					.send(
						`**<@${voted.id}> ${
							death_ids.includes(voted.id)
								? c_death_messages[death_ids.indexOf(voted.id)]
								: death_messages[
										Math.floor(
											Math.random() *
												death_messages.length
										)
								  ]
						}**`
					)
					.then(
						async () =>
							await elim_channel.guild.members.cache
								.get(voted.id)
								.roles.remove("600137520306716707")
					)
					.then(
						async () =>
							await elim_channel.guild.members.cache
								.get(voted.id)
								.roles.add("600137819876360222")
					)
					.then(async () => await db.deleteAll())
					.catch(
						async () =>
							await interaction.channel.send(
								`There was a tie! Make sure to revote.`
							)
					);
			});
            */
