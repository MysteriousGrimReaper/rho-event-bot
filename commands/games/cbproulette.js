...
module.exports = {
	data: new SlashCommandBuilder()
		.setName("cbpr")
		.setDescription(
			"Begin 1 round of Bomb Party Roulette! For event hosts only."
		)
	...
	async execute(interaction) {
			...
			if (Date.now() - i_time > min_sec * 1000) {
				...
				if (
					...
					if (
						...
						game_channel.send(
							`💥 The bomb exploded on <@${bomb.id}>! Better luck next time...`
						);
					}
					game.stop();
				}
		...
		const filter = (check) =>
			array.includes(
				check.content.toUpperCase().replaceAll(/[^A-Z]/gi, "") + "\r"
			) && check.author.id != "1041070495119966358";
		// filter
			...
			if (m.author.id === bomb.id) {
				if (m.content.toUpperCase().includes(sub)) {
					...
					await game_channel.send(
						"Ping someone to throw the bomb at!"
					);
				} else {
					m.react("❌");
				}
		...
		game.on("ignore", async (m) => {
			if (m.content == "fs" && m.author.id == interaction.user.id) {
				...
				m.author.id == bomb.id &&
				m.mentions.members.first().id != `1041070495119966358`
			) {
