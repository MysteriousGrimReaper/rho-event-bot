...
module.exports = {
	...
	/**
	 *
	 * @param {SignupOptions} {interaction: base interaction of the command}
	 *
	 * @returns
	 */
	async create_signup({
		...
		return new Promise((resolve, reject) => {
			try {
				...
				const player_list = []; // array that holds the player list. person who starts is host
				const submissions_list = [];
				const player_name_list = () => {
					return player_list.length == 0
						? [`*No players yet! Join to be the first one...*`]
